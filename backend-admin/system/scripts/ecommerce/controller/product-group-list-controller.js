function ProductGroupListController() {
	this.init = init;
	this.moduleId = null;
	this.oTable = null;
//	this.txtCategoryNameFirst = "";
	...
	function init() {
		...
		if(window.location.search != "") {
			...
			LOGIC.Controllers.ProductGroupListController.moduleId = moduleId;
			...
			ProductGroupListController.prototype.generateTree();
		}

		/*$('#txtSearch').keypress(function(e) {
		 if(e.which == 13) {
		 LOGIC.Controllers.ProductGroupListController.oTable.DataTable().ajax.reload();
		 }
		 });*/
		...
		$(".displayAttr").live("mouseover", function(){
			ProductGroupListController.prototype.displayAttr(this);
		});
			...
			success: function(data, textStatus, xhr) {
				if (data.p0 == '1') {
					var actionHtml =
						...
						'	</div>' +
						*/
						'	<a href="javascript:LOGIC.Controllers.ProductGroupListController.displayAll();" class="btn blue btn-circle" style="margin-right:20px;">' +
						'		<span>显示所有产品 </span>' +
						'	</a>' +
				...
				// create data table
				LOGIC.Controllers.ProductGroupListController.createTable(data);
			},
	        ...
	        LOGIC.Services.StoreService.selectIdByParentOrgId({
				...
				success: function(data, textStatus, xhr) {
					if(data.STATUS != -1 && data != null){
						...
						LOGIC.Controllers.ProductGroupListController.oTable.DataTable().draw(false);
					}
				},
				error: function(jqXHR) {
					LOGIC.ToastrPrompt.toastrError('error');
...
ProductGroupListController.prototype.createTable = function(permission) {
	...
	var columns = [
		...
		},
		{
			data: "", searchable: false, orderable: false, render: function(data1, type, full, meta) {
				var topMoveHtml = '<li id="order_id'+full.orderId+'"><a onclick="LOGIC.Controllers.ProductGroupListController.topMove(' +
				'\'' + full.orderId + '\''+
				')"><i class="fa fa-arrow-circle-up"></i>&nbsp;置顶</a></li>';
				var upMoveHtml = '<li id="order_id'+full.orderId+'"><a onclick="LOGIC.Controllers.ProductGroupListController.upMove(' +
				'\'' + full.orderId + '\', ' +
				'\'' + full.beforeNumber + '\'' +
				')"><i class="fa fa-arrow-circle-up"></i>&nbsp;上移</a></li>';
				var downMoveHtml = '<li id="order_id'+full.orderId+'"><a onclick="LOGIC.Controllers.ProductGroupListController.downMove(' +
				'\'' + full.orderId + '\', ' +
				'\'' + full.afterNumber + '\'' +
...
//			var tem_pagenum = request.start / request.length + 1;
//			//停留当前页
//			if(tem_pagenum == 1){
//				queryOption.pagination.current = order_pagenum;
//			}else {
//				order_pagenum = tem_pagenum;
//				queryOption.pagination.current = order_pagenum;
//			}
//			alert(queryOption.pagination.current);
...
//			queryOption.orderBy.type = 'desc';
			...
			queryOption.queryCriterias = ProductGroupListController.prototype.query_Criterias() ;
...
ProductGroupListController.prototype.query_Criterias = function(){
...
//		queryCriteria.key="category_name";
//		queryCriteria.condition="like";
//		queryCriteria.value=LOGIC.Controllers.ProductGroupListController.txtCategoryNameFirst;
//		queryCriteria.isValueADigital=false;
//		
//		queryCriterias.push(queryCriteria);
//	}
...
ProductGroupListController.prototype.displayAll = function(){
	sessionStorage.removeItem("productParentId");
	LOGIC.Controllers.ProductGroupListController.oTable.DataTable().ajax.reload();
}

ProductGroupListController.prototype.generateTree = function() {
...
ProductGroupListController.prototype.displayAttr = function(element){
...
ProductGroupListController.prototype.upMove = function(orderId, beforeNumber) {
...
ProductGroupListController.prototype.downMove = function(orderId, afterNumber) {
	if(afterNumber=='null'){
...
ProductGroupListController.prototype.topMove = function(orderId) {
	bootbox.setLocale("zh_CN");
	bootbox.confirm({
		size: 'small',
		message: '确定置顶吗？',
